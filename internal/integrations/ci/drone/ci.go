...
func (g *DroneCI) Setup() ([]byte, error) {
	...
	workflowYAML, err := g.GetDroneCiYAML()
...
func (g *DroneCI) GetDroneCiYAML() ([]byte, error) {
	contentsMap := make(map[string]interface{})

	contentsMap["kind"] = "pipeline"
	contentsMap["type"] = "kubernetes"
	contentsMap["name"] = "default"

	contentsMap["steps"] = g.getDroneCiSteps()

	fmt.Println(fmt.Sprintf("[DEBUG] Content: %s", contentsMap))

	return yaml.Marshal(contentsMap)
}

func (g *DroneCI) getDroneCiSteps() []yaml.MapSlice {
	var steps []yaml.MapSlice

	deployPorterStep := yaml.MapSlice{}

	deployPorterStep = append(deployPorterStep,
		yaml.MapItem{
			Key:   "name",
			Value: "Deploy to Porter",
		},
		yaml.MapItem{
			Key:   "image",
			...
			Value: []string{
				fmt.Sprintf(
					"porter update --host \"%s\" --project %d --cluster %d "+
						"--token \"$%s\" --app \"%s\" "+
						"--tag \"$(echo $DRONE_COMMIT_SHA | cut -c1-7)\" --namespace \"%s\" --stream",
	...
	steps = append(steps, deployPorterStep)

	return steps
}

func (g *DroneCI) getClient() (*github.Client, error) {
...
func (g *DroneCI) deleteGithubSecret(
	client *github.Client,
	secretName string,
) error {
...
func (g *DroneCI) CreateEnvSecret() error {
	client, err := g.getClient()
...
func (g *DroneCI) createEnvSecret(client *github.Client) error {
...
func (g *DroneCI) getWebhookSecretName() string {
	return fmt.Sprintf("WEBHOOK_%s", strings.Replace(
		strings.ToUpper(g.ReleaseName), "-", "_", -1),
	)
}

func (g *DroneCI) getBuildEnvSecretName() string {
	return fmt.Sprintf("ENV_%s", strings.Replace(
		strings.ToUpper(g.ReleaseName), "-", "_", -1),
	)
}

func (g *DroneCI) getPorterYMLFileName() string {
	return ".drone.yml"
	if g.InstanceName != "" {
...
func (g *DroneCI) getPorterTokenSecretName() string {
	if g.InstanceName != "" {
